...
wire [3:0] w_ALU_Op_Out;
wire w_IsFP, w_IsACCUM, w_Halt; //NEW
...
wire w_alu_zero;
reg EX_ALUSrc, EX_MemtoReg, EX_RegWrite, EX_MemRead, EX_MemWrite, EX_Branch_ifEqual, EX_Branch_ifNotEqual, EX_IsFP, EX_IsACCUM, EX_Halt;
//reg [1:0] EX_ALUOp;
...
control_unit main_control (
    ...
    .ALUOp1(w_ALUOp1),
    .ALUOp0(w_ALUOp0),
	 .ALU_Op_Out(w_ALU_Op_Out),// NEW
    .IsFP(w_IsFP),        // NEW
    .IsACCUM(w_IsACCUM),  // NEW
    .Halt(w_Halt)         // NEW
);
...
// =============================
assign ID_XD_wire = (ID_Rs == 0) ? 64'd0 : reg_file[ID_Rs];
assign ID_Yd_wire = (ID_Rt == 0) ? 64'd0 : reg_file[ID_Rt];
assign ID_Zd_wire = (ID_Rd == 0) ? 64'd0 : reg_file[ID_Rd];
...
alu_64 theALU (
    .A(EX_operand_A), // No forwarding, use EX_XD directly
	 .ALUOp(EX_ALUOp),
    .B(EX_operand_B),
	 .ACC_in(EX_Zd), // 3rd source Reg used for MULA
    .byte_select(8'b0),
    .ShiftA(7'b0),
    .ShiftB(7'b0),
    .O(w_alu_result),
...
assign BTA = EX_PC_OUT + EX_imm_extended[8:0];
...
begin
    ...
    WB_INSTR <= 0;
	 ...
	 ID_IsFP <= 0; ID_IsACCUM <= 0; ID_Halt <= 0;
	 EX_IsFP <= 0; EX_IsACCUM <= 0; EX_Halt <= 0;
	 MEM_IsFP <= 0;
	 WB_IsFP <= 0;
end
else
begin
    ...
    // =============================
    if (EX_Halt) begin
		 PC_OUT <= PC_OUT;   // freeze PC
	 end else if (Br1) begin
	 	 PC_OUT <= BTA;
	 end else begin
		 PC_OUT <= PC_OUT + 1;
	 end
    ...
    ID_ALUOp          <= w_ALU_Op_Out;
    ID_INSTR          <= instr_mem_douta;
	 ID_IsFP    <= w_IsFP;
	 ID_IsACCUM <= w_IsACCUM;
	 ID_Halt    <= w_Halt;
