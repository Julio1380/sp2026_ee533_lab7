...
module cpu_CMT (
    input CLK, RSTB,
    input [8:0] debug_pc, input debug_enable, input [31:0] debug_instr_in, input debug_instr_write_en,
    output [31:0] debug_instr_out, input [7:0] debug_addr_in, input [63:0] debug_data_in,
    input debug_data_write_en, output [63:0] debug_data_out, output [8:0] PC_END
);
...
reg [63:0] EX1_mult_out; // <--- ADDED: Passes down instantly
...
// DATAPATH LOGIC
// =========================================================================
instr_mem_dp ICache (.addra(PC_OUT), .clka(CLK), .douta(instr_mem_douta), .ena(1'b1), .clkb(CLK), .addrb(debug_pc[8:0]), .dinb(debug_instr_in[31:0]), .doutb(debug_instr_out[31:0]), .enb(debug_enable), .web(debug_instr_write_en));
control_unit main_control (.opcode(instr_mem_douta[31:26]), .RegDst(w_RegDst), .ALUSrc(w_ALUSrc), .MemtoReg(w_MemtoReg), .RegWrite(w_RegWrite), .MemRead(w_MemRead), .MemWrite(w_MemWrite), .Beq(w_Branch_ifEqual), .Bne(w_Branch_ifNotEqual), .ALUOp1(), .ALUOp0(), .ALU_Op_Out(w_ALU_Op_Out), .IsFP(w_IsFP), .IsACCUM(w_IsACCUM), .Halt(w_Halt));
sign_extend sign_ext (.in(EX0_imm), .out(EX0_imm_extended));
assign PC_END = PC_OUT;

// EX0
wire [63:0] w_alu_result; wire w_alu_zero;
alu_64 theALU (.A(EX0_operand_A), .ALUOp(EX0_ALUOp), .B(EX0_operand_B), .O(w_alu_result), .N(), .V(), .C(), .Z(w_alu_zero));
...
// MULTIPLIER NOW SITS HERE IN EX0
...
// MEM & WB
data_mem_64_256 DCache (.addra(MEM_ALU_OUT[7:0]), .clka(CLK), .dina(MEM_STORE_DATA), .douta(data_mem_douta), .ena(MEM_MemRead | MEM_MemWrite), .wea(MEM_MemWrite), .clkb(CLK), .addrb(debug_addr_in[7:0]), .dinb(debug_data_in[63:0]), .enb(debug_enable), .web(debug_data_write_en), .doutb(debug_data_out[63:0]));
assign WB_RData = WB_MemtoReg ? data_mem_douta : (WB_IsFP ? WB_FP_OUT : WB_ALU_OUT);


// =========================================================================
// SEQUENTIAL PIPELINE LOGIC (WITH FULL RESETS)
// =========================================================================
always @(posedge CLK or negedge RSTB) begin
    if (!RSTB) begin
        ...
        // CATCHING THE FAST PIPELINED MULTIPLIER RESULTS HERE
